...
def readParser():
    ...
    parser.add_argument('--delta_percent', type=float, default=0.25, metavar='A',
    ...
    for epoch_step in range(args.num_epoch):
        ...
        for i in count():
            ...
            if total_step % 1000 == 0:
                ...
                #         write = csv.writer(f1)
                #         write.writerows(decay_weights_list)
...
def train_predict_model(args, env_pool, predict_env, logger):
    ...
    if 'decay' in args.reweight_model:
        ...
        decay_weights = None
    if 'delta' in args.reweight_model:
        delta_weights = np.array(env_pool.delta_weights)
    else:
        delta_weights = None
    predict_env.model.train(inputs, labels, decay_weights=decay_weights, delta_weights=delta_weights, batch_size=256,
