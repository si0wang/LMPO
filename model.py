...
class EnsembleDynamicsModel():
    ...
    def train(self, inputs, labels, decay_weights=None, delta_weights=None, batch_size=256, holdout_ratio=0.,
        ...
        if delta_weights is not None:
            delta_weights = delta_weights[permutation]
            train_delta_weights = delta_weights[num_holdout:]
        ...
        for epoch in itertools.count():
            ...
            for start_pos in range(0, train_inputs.shape[0], batch_size):
                ...
                elif delta_weights is not None:
                    idx = np.vstack(
                        [np.array(list(WeightedRandomSampler(train_delta_weights, batch_size, replacement=True)))
                         for _ in range(self.network_size)])
                else:
